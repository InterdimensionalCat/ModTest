...
import com.benthom123.test.particle.ParticleCircleSmoke;
...
/*        motionX *= theEntity.world.rand.nextGaussian() * 0.02D;
    	...
    	if (theEntity.getEntityWorld().isRemote) {
    		Particle particleMysterious = new HealingParticle(theEntity.getEntityWorld(), posX, posY, posZ, motionX, motionY, motionZ, 1.0F);
    		Minecraft.getMinecraft().effectRenderer.addEffect(particleMysterious);
    	}
    }
    
    public void generateVoidParticle(Entity theEntity, EntityVortexField vortex, double posX, double posY, double posZ, double motionX, double motionY, double motionZ, double angleIn, double radiusIn)
    {
    	if (theEntity.getEntityWorld().isRemote) {
    		Particle particleMysterious = new ParticleCircleSmoke(theEntity.getEntityWorld(), posX, posY, posZ, motionX, motionY, motionZ, 1.0F, vortex, angleIn, radiusIn);
    		Minecraft.getMinecraft().effectRenderer.addEffect(particleMysterious);
    	}
    }
 ...
 /*   	  event.getRegistry().register(new CopperIngot());
    	  ...
    	  //MAGIC
    	  event.getRegistry().register(new SmokeShot("smokeshot"));
    	  event.getRegistry().register(new EmptyCrystal("emptycrystal"));
    	  event.getRegistry().register(new VoidRift("voidrift"));
    	  event.getRegistry().register(new GlacialSpear("glacialspear"));
    	  event.getRegistry().register(new HealingRift("healingrift"));
    	  event.getRegistry().register(new EmptyCrystal("invisible"));
    	  event.getRegistry().register(new VortexField("vortexfield"));
    	  ...
    	  event.getRegistry().register(new TwoTwentyThreeRemington("tttremington"));
    	  event.getRegistry().register(new ArmaLite15("arfifteen"));
