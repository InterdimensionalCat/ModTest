...
public class ModSword extends ItemSword {
	...
	public ModSword(String registryName, ToolMaterial material, double attackDamage, double attackSpeed, int repairIndex, CreativeTabs tab, double kickback, boolean customKnockback, boolean damage, boolean speed) {
		super(material);
	    ...
	    private void replaceModifier(Multimap<String, AttributeModifier> modifierMultimap, IAttribute attribute, UUID id, double multiplier)
	    {
		 ...
		 public boolean hitEntity(ItemStack stack, EntityLivingBase target, EntityLivingBase wielder) {
		     ...
		     if ((!customKnockback)&&(kickbackModifier != 0.0D)) {
					...
					if (customKnockback&&(this.materialID == CommonProxy.TITANIUM)) {
					     Vec3d look = wielder.getLookVec().normalize();
					     if (look.y < 0) {
						...
						if (customKnockback&&(this.materialID == CommonProxy.PLATINUM)) {
						     Vec3d look = wielder.getLookVec().normalize();
						     if (look.y < 0) {
