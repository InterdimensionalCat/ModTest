...
public class HealingRift extends PowerCrystal {
	
	BlockPos playerpos;
	int maxDamage;
	EntityPlayer user;

	public HealingRift(String name) {
		super(name);
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
		...
		if (itemstack.getItemDamage() == 0) {
			if (!worldIn.isRemote) {
				...
				makeHealingRift(itemstack, PotionTypes.HEALING, playerIn, worldIn);
			}
			...
			if (playerIn.experienceLevel <= 32) {
				this.setMaxDamage(300 - (int) ((349 / 30) * playerIn.experienceLevel) + 400);
			} else {
				this.setMaxDamage(300 - (int) ((349 / 30) * 32) + 400);
			}
			//maxDamage = 100 - playerIn.experienceLevel;
			itemstack.damageItem(300 - (int) ((349 / 30) * playerIn.experienceLevel) - 1 + 400, playerIn);
			
			
			worldIn.playSound(playerIn, playerIn.posX, playerIn.posY, playerIn.posZ, SoundEvents.BLOCK_NOTE_BELL, SoundCategory.NEUTRAL,  0.7F, 0.71F);
			worldIn.playSound(playerIn, playerIn.posX, playerIn.posY, playerIn.posZ, SoundEvents.BLOCK_NOTE_BELL, SoundCategory.NEUTRAL,  0.7F, 0.9F);
			worldIn.playSound(playerIn, playerIn.posX, playerIn.posY, playerIn.posZ, SoundEvents.BLOCK_NOTE_BELL, SoundCategory.NEUTRAL,  0.7F, 1.06F);
...
/*			this.playerpos = playerIn.getPosition();
	...
	private void makeHealingRift(ItemStack item, PotionType type, EntityPlayer playerIn, World worldIn)
	{
		BlockPos pos = playerIn.getPosition();
		EntityHealingRift voidrift = new EntityHealingRift(worldIn, pos.getX() , pos.getY(), pos.getZ());
	    voidrift.setOwner(playerIn);
	    //voidrift.setRadius(3.0F);
	    //voidrift.setRadiusOnUse(-0.5F);
	    //voidrift.setWaitTime(10);
	    voidrift.setRadiusPerTick((voidrift.getRadius() / (float)voidrift.getDuration())*5);
